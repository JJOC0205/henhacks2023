...
with ui.tab_panels(tabs, value="Home"):
    ...
    with ui.tab_panel("About"):
        ...
        ui.markdown("Through the Canvas API, we produce a list of users in your classes with their UD email.")
        ui.markdown("No longer will you struggle to find group members or miss out on valuable friendships!")
        ui.markdown("###Future Goals:")
        ui.markdown("- Allow user descriptions for matches to read")
        ui.markdown("- Create a more user-friendly UI")
    with ui.tab_panel("Help"):
        ...
        ui.label("Scroll to the Approved Integrations section [1]. For each access token, you can view the name [2], purpose [3], expiration date [4], and date of last use [5]").style("font-size: 200%")
        ...
        ui.label("to manually add an access token, click the Add New Access Token button.").style("font-size: 200%")
        ...
        ui.label("Enter a description for your access token in the Purpose field [1]. You can also select an expiration date by clicking the Calendar icon [2]. To generate a token with no expiration, leave the Expires field empty. To generate a new access token, click the Generate Token button [3].").style("font-size: 200%")
        ...
        ui.label("The access token details include a token that can be used to make API calls on your behalf [1]. To regenerate an access token, click the Regenerate Token button [2].").style("font-size: 200%")
